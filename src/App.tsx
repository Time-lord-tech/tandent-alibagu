...
import {
  ...
  Star, Sparkles, Menu, X, MessageCircle
} from 'lucide-react';
              ...
              >
                ...
                <div className="services-progress">
                  {services.map((_, i) => (
                    <div
                      key={i}
                      className={`services-progress-dot ${i === activeIndex ? 'active' : 'inactive'}`}
                    />
                  ))}
                </div>
              </motion.div>
      ...
      >
        ...
        <div className="timeline-steps">
          <div
            ...
            style={{ width: `calc(${beamWidth}% - 8rem)` }}
          />
          {timelineSteps.map((step, i) => (
            ...
            >
              <div className={`timeline-step-num ${i < litCount ? 'lit' : ''}`}>
                {step.num}
              </div>
              <h4>{step.title}</h4>
              <p>{step.desc}</p>
    ...
    <section id="testimonials" className="testimonials">
      <div className="testimonials-inner">
        ...
        >
          ...
          <h2 className="section-title" style={{ color: 'var(--charcoal)', margin: '0 auto', textAlign: 'center' }}>
            Patients who were scared, tell it best.
          </h2>
        </motion.div>
            ...
            >
              ...
              <div className="testimonial-stars">
                {Array.from({ length: r.rating }).map((_, j) => (
                  <Star key={j} size={14} fill="#C9A96E" />
                ))}
              </div>
    ...
    <section id="contact" className="contact">
      <div className="contact-inner">
        ...
        >
          ...
          <h2 className="section-title" style={{ color: 'var(--charcoal)' }}>
            Ready for a judgment-free visit?
          </h2>
            ...
            {[
              ...
              <div key={i} className="contact-info-item">
                ...
                <div>
                  ...
                  {item.link ? (
                    <a href={item.link} target={item.link.startsWith('http') ? "_blank" : "_self"} rel="noreferrer" className="contact-info-link">
                      {item.value}
                    </a>
                  ) : (
            ...
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                  ...
                  <input required type="email" className="form-input" placeholder="Where should we send confirmation?" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                </div>
                <div className="form-group">
                  <label>Phone / FB Messenger</label>
                  <input required type="text" className="form-input" placeholder="How can we reach you?" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                </div>
                <div className="form-group">
                  <label>Message</label>
                  <textarea className="form-input" placeholder="Questions, concerns, or just say hello..." value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} />
                </div>
                <button type="submit" disabled={status === 'loading'} className="form-submit">
                  {status === 'loading' ? 'Sending...' : 'Send Message'}
                </button>
                {status === 'error' && <p style={{ color: 'red', marginTop: '1rem', fontSize: '0.9rem' }}>Something went wrong. Please try again.</p>}
              </form>
            )}
          </div>
        </motion.div>
  ...
  return (
    <div style={{ padding: '0 2rem 6rem', maxWidth: '1248px', margin: '0 auto' }}>
      ...
      >
        ...
        <a href="#contact" className="btn-primary">
          Get Instant SMS Confirmation <ArrowRight size={16} />
        </a>
      </motion.div>
    </div>
  );
          ...
          {team.map((member, i) => (
            ...
            >
              <div className="team-image">
                ...
                {member.permit && <div className="team-permit-badge">{member.permit}</div>}
              </div>
